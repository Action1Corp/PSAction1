...
The upload is performed using a **resumable chunked upload process**.

Chunked uploads allow large files to be transferred reliably and provide progress feedback during the upload process.

The upload platform must be specified (`Windows_32` or `Windows_64`). The chunk size can be controlled using the `BufferSize` parameter.
