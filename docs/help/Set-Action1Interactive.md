...
When interactive mode is enabled, the PSAction1 module may prompt the user to provide values for required variables that are not already set. This helps guide users through configuration when running commands that depend on module-level variables.

When interactive mode is disabled, commands will not prompt for missing values
and may fail if required variables are not set.
